...
public class RegexCatenation<T> extends RegexIntNode<T> {
	// Only children
	public RegexCatenation(RegexNode<T> left, RegexNode<T> right) {
		super(null, Arrays.asList(left, right));
	}
	
	public RegexCatenation(List<RegexNode<T>> subs) {
		super(subs);
	}
	
	// Parent & children
	public RegexCatenation(RegexNode<T> left, RegexNode<T> right, RegexIntNode<T> parent) {
		super(parent, Arrays.asList(left, right));
	}
	
	public RegexCatenation(List<RegexNode<T>> subs, RegexIntNode<T> parent) {
		super(parent, subs);
	}
	...
	public RegexNode<T> deepCopy() {
		return new RegexCatenation<T>(left().deepCopy(), right().deepCopy());
	}
